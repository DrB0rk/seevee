...
assert "template SDK compiled entry present" test -f "$BUNDLE_DIR/runtime/template-sdk/dist/index.js"
assert "runtime/renderer/"     test -d "$BUNDLE_DIR/runtime/renderer"
assert "renderer compiled entry present" test -f "$BUNDLE_DIR/runtime/renderer/dist/index.js"
...
assert "no bundled Claude platform binary" sh -c "! find '$BUNDLE_DIR/runtime/node_modules/.pnpm' -maxdepth 1 -type d -name '@anthropic-ai+claude-agent-sdk-*' -print -quit | grep -q ."
assert "runtime/node_modules/" test -d "$BUNDLE_DIR/runtime/node_modules"
assert "workspace-agent skill installed in bundle" test -f "$BUNDLE_DIR/runtime/agent/seevee-workspace-agent/SKILL.md"
assert "workspace-agent references installed in bundle" test -d "$BUNDLE_DIR/runtime/agent/seevee-workspace-agent/references"
assert "default Classic template installed in bundle" test -f "$BUNDLE_DIR/runtime/templates/classic/v1/template.json"
